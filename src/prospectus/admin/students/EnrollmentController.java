...
public class EnrollmentController implements Initializable {
    ...
    private void submitEnrollmentHandler(MouseEvent event) {
        ...
        String enrollmentStatus = getEnrollmentStatus(userId);
        if (enrollmentStatus != null) {
            if ("Enrolled".equalsIgnoreCase(enrollmentStatus)) {
                utilities.showAlert(Alert.AlertType.INFORMATION, "Already Enrolled", "You are already enrolled in the system.");
                return;
            } else if ("Pending".equalsIgnoreCase(enrollmentStatus)) {
                utilities.showAlert(Alert.AlertType.INFORMATION, "Enrollment in Process", "Your enrollment is still being processed. Please wait for admin approval.");
                return;
            }
        }
    ...
    private String getEnrollmentStatus(int userId) {
        String query = "SELECT enrollment_status FROM user WHERE u_id = ?";
        try (Connection conn = db.getConnection();
             PreparedStatement pst = conn.prepareStatement(query)) {
            pst.setInt(1, userId);
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
                return rs.getString("enrollment_status");
            }
        } catch (SQLException ex) {
            utilities.showAlert(Alert.AlertType.ERROR, "Database Error", "Failed to check enrollment status: " + ex.getMessage());
        }
        return null;
    }
